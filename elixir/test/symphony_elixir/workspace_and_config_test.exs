defmodule SymphonyElixir.WorkspaceAndConfigTest do
  ...
  test "config reads defaults for optional settings" do
    ...
    assert config.claude_code.read_timeout_ms == 30_000
    assert config.claude_code.stall_timeout_ms == 300_000

    write_workflow_file!(Workflow.workflow_file_path(),
      claude_code_command: "claude --foo --bar",
      claude_code_permission_mode: "acceptEdits",
      claude_code_model: "claude-opus-4-7",
      claude_code_extra_args: ["--baz"]
    )

    config = Config.settings!()
    assert config.claude_code.command == "claude --foo --bar"
    assert config.claude_code.permission_mode == "acceptEdits"
    assert config.claude_code.model == "claude-opus-4-7"
    assert config.claude_code.extra_args == ["--baz"]
    ...
    write_workflow_file!(Workflow.workflow_file_path(), claude_code_read_timeout_ms: "bad")
    assert {:error, {:invalid_workflow_config, message}} = Config.validate!()
    assert message =~ "claude_code.read_timeout_ms"

    write_workflow_file!(Workflow.workflow_file_path(), claude_code_stall_timeout_ms: "bad")
    assert {:error, {:invalid_workflow_config, message}} = Config.validate!()
    assert message =~ "claude_code.stall_timeout_ms"

    write_workflow_file!(Workflow.workflow_file_path(), claude_code_permission_mode: "definitely-not-valid")
    assert {:error, {:invalid_workflow_config, message}} = Config.validate!()
    assert message =~ "claude_code.permission_mode"
  end
  ...
  test "config resolves $VAR references for env-backed secret and path values" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      ...
      claude_code_command: "#{claude_bin}"
    )

    config = Config.settings!()
    assert config.tracker.api_key == api_key
    assert config.workspace.root == Path.expand(workspace_root)
    assert config.claude_code.command == "#{claude_bin}"
  end
  ...
  test "schema parse falls back env-backed values for tracker secrets and workspace root" do
  ...
  test "claude_code runtime settings expose typed runtime config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-claude-code-runtime-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        ...
        claude_code_permission_mode: "acceptEdits",
        claude_code_model: "claude-opus-4-7",
        claude_code_extra_args: ["--mcp-config", "mcp.json"]
      )

      assert {:ok, runtime} = Config.claude_code_runtime_settings(workspace_root)
      assert runtime.command == "claude --dangerously-skip-permissions"
      assert runtime.permission_mode == "acceptEdits"
      assert runtime.model == "claude-opus-4-7"
      assert runtime.extra_args == ["--mcp-config", "mcp.json"]
      assert runtime.turn_timeout_ms == 3_600_000
      assert runtime.read_timeout_ms == 30_000
      assert runtime.stall_timeout_ms == 300_000
    after
      File.rm_rf(test_root)
    end
  end
