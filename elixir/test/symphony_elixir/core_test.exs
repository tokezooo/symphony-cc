defmodule SymphonyElixir.CoreTest do
  ...
  test "config defaults and validation checks" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(), claude_code_command: "claude --foo")
    assert :ok = Config.validate!()
    assert Config.settings!().claude_code.command == "claude --foo"

    write_workflow_file!(Workflow.workflow_file_path(), claude_code_permission_mode: "unsupported")
    assert {:error, {:invalid_workflow_config, message}} = Config.validate!()
    assert message =~ "claude_code.permission_mode"

    write_workflow_file!(Workflow.workflow_file_path(), claude_code_permission_mode: "acceptEdits")
    assert :ok = Config.validate!()

    write_workflow_file!(Workflow.workflow_file_path(), claude_code_turn_timeout_ms: 0)
    assert {:error, {:invalid_workflow_config, message}} = Config.validate!()
  ...
  test "in-repo WORKFLOW.md renders correctly" do
    ...
    assert prompt =~ "follow the configured merge flow"
